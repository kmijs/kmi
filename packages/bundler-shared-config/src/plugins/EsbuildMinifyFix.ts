...
import { MagicString, remapping } from '@kmijs/shared'
    ...
    compiler.hooks.compilation.tap(this.name, (compilation: Compilation) => {
      compilation.hooks.processAssets.tapPromise(
        ...
        (assets: Record<string, sources.Source>) =>
          this.minifyFix(compilation, assets, compiler.options.devtool),
      )
    })
  }
    ...
    const assetsForMinify = await Promise.all(
      Object.keys(assets)
        .filter((name) => {
          ...
          // 如果存在 sourceMap 则不处理
          if (info?.related?.sourceMap && devtool !== 'hidden-source-map') {
            return false
          }
