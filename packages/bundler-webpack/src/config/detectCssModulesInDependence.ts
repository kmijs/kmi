...
export async function addDependenceCssModulesDetector(opts: IApplyOpts) {
  const { config, cwd, userConfig } = opts

  if (!userConfig.checkDepCssModules) return

  const matchers = opts.extraBabelIncludes.map((p) => {
...
class DetectCSsModulePlugin {
  ...
  private isJSModule(parser: any): boolean {
    const res = parser.state?.module?.resource
    if (res) {
  ...
  private throwError(file: string, c: Compiler) {
    ...
    logger.error(picocolors.red(`Dependence file ${file} contains css module`))
    logger.error(
      picocolors.red(
