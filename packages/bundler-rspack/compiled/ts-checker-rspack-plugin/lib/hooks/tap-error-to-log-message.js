    ...
    hooks.error.tap('TsCheckerRspackPlugin', (error) => {
        ...
        if (error instanceof rpc_1.RpcExitError) {
            if (error.signal === 'SIGINT') {
                config.logger.error(picocolors_1.default.red('Issues checking service interrupted - If running in a docker container, this may be caused ' +
                ...
                config.logger.error(picocolors_1.default.red('Issues checking service aborted - probably out of memory. ' +
