import resolve from '@kmijs/shared/compiled/resolve'
...
export async function buildDep(opts: any): Promise<void> {
  ...
  if (opts.pkgName) {
    ...
    if (opts.pkgName === '@gitbeaker/rest') {
      resolvePath = '@gitbeaker/rest/dist/index.js'
    }
  ...
  Object.defineProperty(exports, key, {
    ...
    get: function get() {
      ...
      if (opts.file === './bundles/babel/bundle') {
        delete opts.webpackExternals['@babel/core']
        ...
        const babelCorePkg = require.resolve('@babel/core/package.json', {
          paths: [path.join(PATHS.PACKAGES, './bundler-compiled')],
        })
        ...
        const nodePartFile = path.join(path.dirname(browserslistPkg), 'node.js')
      ...
      // @babel/highlight 文件多了一层
      if (opts.pkgName === '@babel/highlight') {
        // opts.webpackExternals = Object.keys(opts.webpackExternals).reduce<
        //   Record<string, string>
        // >((acc, key) => {
        //   const val = opts.webpackExternals[key]
        //   if (val.startsWith('../')) {
        //     acc[key] = val.replace('../', '../../')
        //   } else {
        //     acc[key] = val
      ...
      if (opts.pkgName === '@clack/core') {
        opts.webpackExternals = {
          sisteransi: '../../sisteransi',
        }
      }

      if (opts.pkgName === '@clack/prompts') {
        opts.webpackExternals = {
          '@clack/core': '../core',
          sisteransi: '../../sisteransi',
        }
      ...
      // 处理 update-notifier
      if (opts.pkgName === 'update-notifier') {
        code = code.replace(
          'require("@kmijs/shared/compiled/boxen");',
          'require("@kmijs/shared/compiled/boxen").default;',
        )
      }
      ...
      if (opts.pkgName === 'vite') {
        code = code.replace(
          ...
          'loadPreprocessor("@kmijs/bundler-shared/compiled/less"',
        )
      }

      if (opts.file === './bundles/webpack/bundle') {
        const pkg = require(path.join(opts.base, 'package.json'))
        const kmiVersion = pkg.version
        console.log('kmiVersion', kmiVersion)
        // webpack 版本打包不正确
        code = code
          .replaceAll(
            `{"version":"${kmiVersion}"}`,
            `{"version":"${pkg.devDependencies.webpack}"}`,
          )
          .replaceAll(
            `version:"${kmiVersion}"`,
            `version:"${pkg.devDependencies.webpack}"`,
        ...
        code = code.replaceAll(
          `error occurred: ${kmiVersion}`,
          `error occurred: ${pkg.devDependencies.webpack}`,
        )

        if (code.includes(kmiVersion)) {
          throw new Error(
            `${opts.pkgName} webpack 版本存在问题，请检查生成的预打包文件！`,
          )
        }
      }
        ...
        readWantedLockfile(PATHS.ROOT, {
          ...
          if (
            ...
            throw new Error(`regenerate-unicode-properties is outdated, please update it to ${
              unicodeParentPkg.dependencies![unicodePkgName]
            } in bundler-compiled/package.json before update compiled files!
  ...
  if (opts.pkgName) {
    if (opts.isDependency) {
      ...
      const { name, author, license, types, typing, typings, version } =
        JSON.parse(fs.readFileSync(path.join(pkgRoot, 'package.json'), 'utf-8'))
        ...
        if (opts.pkgName === 'webpack-5-chain') {
          ...
          fs.writeFileSync(
            ...
            fs
              ...
              .replace(
                `} from 'webpack';`,
                `} from '@kmijs/bundler-webpack/compiled/webpack';`,
              ),
            'utf-8',
          )
        }
        ...
        if (opts.pkgName === 'less') {
          ...
          fs.writeFileSync(
            ...
            fs
              ...
              .replace(
                'declare module "less"',
                'declare module "@kmijs/bundler-shared/compiled/less"',
              ),
            'utf-8',
          )
        }
